...
}).factory('getEmployeeData',function($http){
    return function (params) {
        return $http.get(appUrl+'get_employee_details?token='+localStorage.getItem('token'),{params:params,header:{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8'}
        })
    }
...
}).factory('getAttendanceData',function($http){
    return function (params) {
