...
employeeDetailModule.controller('employeeDetailController',['$rootScope','$scope','$location','$route','$http','commonService','getHrmGeneralMaritalStatusListing',
  'getHrmGeneralDepartmentListing','getHrmGeneralDesignationListing','getHrmGeneralCategoryListing','saveEmployeeDetailService','getStateListService',
  'getHrmSalaryComponentListing','getHrmGeneralBloodGroupListing','getHrmGeneralCompanyEmailListing','getSalaryConfigurationListing','getEmployeeListing',
  'getWarehouseListService','getHrmGeneralProbationPeriodListing','getHrmGeneralNoticePeriodListing','editEmployeeDetailsService','updateEmployeeDetailService',
  'deleteEmployeeDetails','getSalaryConfigurationListing','getLeaveStatus','getLocationListing',
  function($rootScope,$scope,$location,$route,$http,check_permission,getHrmGeneralMaritalStatusListing,getHrmGeneralDepartmentListing,
    getHrmGeneralDesignationListing,getHrmGeneralCategoryListing,saveEmployeeDetailService,getStateListService,
    getHrmSalaryComponentListing,getHrmGeneralBloodGroupListing,getHrmGeneralCompanyEmailListing,getSalaryConfigurationListing,
    getEmployeeListing,getWarehouseListService,getHrmGeneralProbationPeriodListing,getHrmGeneralNoticePeriodListing,editEmployeeDetailsService,
    updateEmployeeDetailService,deleteEmployeeDetails,getSalaryConfigurationListing,getLeaveStatus,
    getLocationListing){
  
  $scope.active_lable = false;
  
  $scope.employee = {'employee_salary_details':{},'employee_asset_details':[],'employee_details':'','leave_details':[]};  
  
  $scope.hide_remaining_leave=true;
 ...
 $scope.comment ={};
  ...
  /**/
  $scope.hold_old_allowance = {};
  $scope.save_allowances = [];
  /**/
  var leave;
  ...
  $scope.get_employee_leave_status=function(id){
    console.log('++++++++++++++++++++++++++',id);
        getLeaveStatus({
          designation_id:id
    ...
    //$scope.get_employee_leave_status();
  ...
  $scope.files = [];
   ...
   $scope.save_employee_details = function(){    
    ...
    for(var y=0;y<$scope.salary_allowances.length;y++){ 
        console.log('$scope.salary_allowances[y].allowance_amount----------->',$scope.salary_allowances[y].allowance_amount);
      /**/
      if($scope.salary_allowances[y].allowance_amount == '' || $scope.salary_allowances[y].allowance_amount == null || $scope.salary_allowances[y].allowance_amount == undefined){
          $scope.salary_allowances[y].allowance_amount = 0;
      }
      /**/
    ...
    //$scope.employee.employee_salary_details = $scope.salary_components;
     ...
     /* $scope.other_deduction.deduction = $scope.other_deductions;
      $scope.other_deduction.deduction_amount = $scope.other_ded;
      $scope.employee.employee_salary_details.tax_deductions.push($scope.other_deduction);*/
    //$scope.employee.employee_salary_details =  $scope.salary_components;

      console.log('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~',leave);

      for(var k=0;k<$scope.leave_status.length;k++){

        angular.forEach($scope.leave_status[k],function(value,key){
                 /*if($scope.leave_status[k][key]!='leave_type_id' || $scope.leave_status[k][key]!='days'){
                    delete $scope.leave_status[k][key];
                  }*/
                  console.log(key);
                  console.log(value);
                  delete $scope.leave_status[k]['created_at'];
                  delete $scope.leave_status[k]['created_date'];
                  delete $scope.leave_status[k]['id'];
                  delete $scope.leave_status[k]['emp_type'];
                  delete $scope.leave_status[k]['updated_at'];
                  delete $scope.leave_status[k]['designation_id'];
                  //delete $scope.leave_status[k]['leave_type'];
            });
      }

    $scope.employee.leave_details = $scope.leave_status;
   ...
   // $scope.pt_tax_deduction ={};
    ...
    var dataObject = {
       ...
       $http({
           method: 'POST',
           url: appUrl+'add_employee?token='+localStorage.getItem('token'),
           ...
           transformRequest: function (data) {
               ...
               for (var i = 0; i < data.files.length; i++) {
                   ...
                   formData.append('uploadimages', data.files[i]);
               }
       ...
       success(function (data, status, headers, config) {
           ...
           console.log("success");
           $('#employeeDetailSuccess').fadeIn().delay(5000).fadeOut();
           $scope.employee_detail_success = JSON.parse(data);
           $scope.onLoadGetEmployeeListing();
       }).
   ...
   $scope.edit_employee_Detail = function(emp_id){
     $scope.temporary = 0;
     $scope.form_field = true;
     $scope.hide_remaining_leave=false;
    ...
    //$scope.employee.employee_id = emp_id;
     editEmployeeDetailsService({employee_id:emp_id}).then(function (response) {
           ...
           // $scope.employee.employee_salary_details = response.data[0].emp_salary_details;
            ...
            $scope.employee.employee_details.emp_password=response.data[0].original_password;
            ...
            //     console.log(response);
            // })
            $scope.get_employee_leave_status(response.data[0].emp_details.designation_id);
                           ...
                           if($scope.salary_allowances.length > 0){
                            ...
                            console.log('$scope.salary_allowances.length-->',$scope.salary_allowances.length);
                             if($scope.salary_allowances[x].status == 'active'){
                               /**/
                                if($scope.salary_allowances[x].allowance_amount == '' || $scope.salary_allowances[x].allowance_amount == null || $scope.salary_allowances[x].allowance_amount == undefined){
                                  $scope.salary_allowances[x].allowance_amount = 0;
                                }
              ...
              for(var k=0;k<response.data[0].allowances.length;k++){
                 if(response.data[0].allowances[k].allowance == 'Others'){
                    ...
                    if(response.data[0].allowances[k].allowance != 'Others' && response.data[0].allowances[k].status == 'active'){
                        ...
                        /**/
                         $scope.salary_allowances[k].status = 'active';
                         $scope.hold_old_allowance.allowance = response.data[0].allowances[k].allowance;
                         $scope.hold_old_allowance.allowance_amount = response.data[0].allowances[k].allowance_amount;
                         $scope.save_allowances.push($scope.hold_old_allowance);
                        /**/
                      }else{
                        $scope.salary_allowances[k].id = false;
                        $scope.salary_allowances[k].allowance_amount = response.data[0].allowances[k].allowance_amount;
                        /**/
                         $scope.salary_allowances[k].status = 'inactive';
                        /**/
                      }
                 }
              }
              console.log('$scope.salary_allowances check status---->',$scope.salary_allowances);
   ...
   $scope.current_modal = "hrm_user";

        $scope.hrm_empdetails_permission_popup = function (supp_id,supp_status,supp_type) {
            check_permission.check_status(supp_id,supp_status,supp_type,$scope.current_modal);
            $scope.comment ={};
            $scope.otp_object = {};
        };
        $scope.hrm_generate_password_function = function () {
            check_permission.generate_otp($scope.comment);
            $scope.comment ={};
        };
        $scope.hrm_verify_otp = function () {
        ...
        $scope.$on('CallParentMethod', function (event, data) {
            if(data.model==$scope.current_modal){
              $scope.onLoadGetEmployeeListing();
                ...
                $scope.edit_employee_Detail(data.id);
                $('#example21').modal('show');
            }
        });
        $scope.$on('updateListing', function (event, data) {
            if(data.model==$scope.current_modal){
                ...
                if(data.otp){
                    $('#employeeDetailSuccess').fadeIn().delay(5000).fadeOut();
                    $scope.employee_detail_success = 'Employee details deleted Successfully';
                }
            }
        });
  ...
  $scope.save_allowances = [];
    ...
    var dataObject = {
       ...
       $http({
           method: 'POST',
           url: appUrl+'update_employee?token='+localStorage.getItem('token'),
           headers: { 'Content-Type': undefined },
           transformRequest: function (data) {
               var formData = new FormData();
               formData.append("employee", angular.toJson(data.model));
                        ...
                        //  $scope.others_cal = $scope.gross_amount - $scope.temporary;
                          ...
                          if($scope.salary_allowances.length>0){
                           for(var x=0;x<$scope.salary_allowances.length;x++){
                            ...
                            console.log('$scope.salary_allowances-->',$scope.salary_allowances);
                             if($scope.salary_allowances[x].status == 'active'){
                              console.log('$scope.salary_allowances[x].allowance_amount--->',$scope.salary_allowances[x].allowance_amount);
                              /**/
                              if($scope.salary_allowances[x].allowance_amount == '' || $scope.salary_allowances[x].allowance_amount == null || $scope.salary_allowances[x].allowance_amount == undefined){
                                $scope.salary_allowances[x].allowance_amount = parseFloat(0);
                              }
                              /**/
                              console.log('others_cal------>',$scope.others_cal);
                               $scope.others_cal = parseFloat($scope.others_cal) - parseFloat($scope.salary_allowances[x].allowance_amount); 
...
*/
         ...
         if(arr[ind].id == true){
          // $scope.salary_allowances[ind].id = arr[ind].id;
           console.log('if called');
            ...
            if($scope.others_cal == 0){
                /**/
                if(arr[ind].allowance_amount == '' || arr[ind].allowance_amount == null || arr[ind].allowance_amount == undefined){
                  arr[ind].allowance_amount = 0;
                }
                ...
                /**/
                 ...
                 if($scope.allowance_cal > 0){
                          ...
                          /**/
                          $scope.hold_old_allowance.allowance = arr[ind].allowance;
                          $scope.hold_old_allowance.allowance_amount = arr[ind].allowance_amount;
                          $scope.save_allowances.push($scope.hold_old_allowance);
                          console.log('$scope.save_allowances--->',$scope.save_allowances);
                           /**/
                       }else{
                         $scope.others = 0;
                          /**/
             ...
             $scope.others=0;
              /**/
                         $scope.hold_old_allowance.allowance = arr[ind].allowance;
                          $scope.hold_old_allowance.allowance_amount = arr[ind].allowance_amount;
                         // $scope.save_allowances.push($scope.hold_old_allowance);
              var index = $scope.save_allowances.indexOf($scope.hold_old_allowance);             
              $scope.save_allowances.splice(index, 1);
                /**/
          }else{
  ...
  /**/
    ...
    $scope.change_allowance_amount = function(arr,ind){
      console.log('$scope.save_allowances----------->',$scope.save_allowances);
      console.log('$scope.salary_allowances--->',$scope.salary_allowances);
       if(arr[ind].id == true){
          if($scope.save_allowances.length >0){
            for(var i=0;i<$scope.save_allowances.length;i++){

               if($scope.save_allowances[i].allowance == arr[ind].allowance){
                
                 if(arr[ind].allowance_amount != ''){
                  console.log('if not blank');
                 $scope.temp_others_cal = parseFloat($scope.others_cal) + parseFloat($scope.save_allowances[i].allowance_amount);
                 $scope.others_cal = $scope.temp_others_cal;
                 console.log('add all $scope.others_cal--->', $scope.others_cal);
               //}
               if($scope.others_cal == 0){
                  $scope.allowance_cal = parseFloat($scope.gross_amount) - parseFloat(arr[ind].allowance_amount);
                 if($scope.allowance_cal > 0){
                          $scope.others = $scope.allowance_cal;
                          $scope.others_cal = $scope.allowance_cal;
                          $scope.save_allowances[i].allowance_amount = arr[ind].allowance_amount;
                       }else{
                         $scope.others = 0;
                         $scope.save_allowances[i].allowance_amount = arr[ind].allowance_amount;
                       }
            ...
            console.log('if  blank');
             console.log('$scope.save_allowances----------->',$scope.save_allowances);
              $scope.temp_others_cal = parseFloat($scope.others_cal) + parseFloat($scope.save_allowances[i].allowance_amount);
              $scope.others_cal = $scope.temp_others_cal;
              $scope.others = $scope.others_cal;
              $scope.save_allowances[i].allowance_amount = 0;
              //arr[ind].allowance_amount = 0;
          }
          ...
          if($scope.salary_allowances.length>0){
             ...
             for(var x=0;x<$scope.salary_allowances.length;x++){
               console.log('$scope.salary_allowances.length-->',$scope.salary_allowances.length);
               //if($scope.salary_allowances[x].status == 'active'){
               if($scope.salary_allowances[x].allowance == arr[ind].allowance){
                /**/
                $scope.salary_allowances[x].allowance_amount = arr[ind].allowance_amount;                
              }
            }
            console.log('$scope.salary_allowances--->',$scope.salary_allowances);
          }
    ...
    /*$scope.change_deduction_amount = function(ded){
        if(ded != ''){
            $scope.others_cal = parseFloat($scope.others_cal) - parseFloat(ded);
            if($scope.others_cal > 0){
                $scope.others = $scope.others_cal;
                console.log('$scope.others_cal-->',$scope.others_cal);
            }else{
...
*/
    ...
    $scope.check_password = function(){
      if($scope.employee.employee_details.emp_password!=$scope.confirm_password){
