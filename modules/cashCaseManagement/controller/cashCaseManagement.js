var cashCaseManagementModule= angular.module('cashCaseManagementModule',['ngBootbox']);
cashCaseManagementModule.controller('cashCaseManagementController',['$rootScope','$scope','$location','$route','$timeout',
    '$filter','$window','getClientList','getServiceList1','getStateListService','vehicleAutoComplete','chargeListService',
    'getTaxRateService','saveCaseManagementData','getCaseDetailsById','updateCaseManagementData','getCashCaseDetailsList','approveCaseStatus','commonService',
    ...
    function($rootScope,$scope,$location,$route,$timeout,$filter,$window,getClientList,getServiceList1,getStateListService,
             ...
             getCashCaseDetailsList,approveCaseStatus,check_permission,saveCaseClosingData,getClientWiseRules,checkCustomerStatus,
    ...
    $scope.get_case_details = function () {
        getCashCaseDetailsList({}).then(function (resposne) {
    ...
    $scope.calculate_tax_amount=function(data){
    ...
    $scope.edit_case_details=function (id,type) {
        ...
        getCaseDetailsById({
            ...
            $scope.case_details.type = response.data[0].type;
            ...
            $scope.case_closing.tax_name=response.data[0].tax_name;
            $scope.case_details.tax_type=response.data[0].tax_type;
    ...
    $scope.update_case_management_data=function () {
        ...
        console.log(JSON.stringify($scope.data.transportaion));
        $scope.case_closing.transportation_charges=$scope.data.transportaion;
        console.log($scope.case_closing.transportation_charges);
        console.log($scope.case_closing.transportation_charges.length);
        ...
        if($scope.case_closing.transportation_charges!=null && $scope.case_closing.transportation_charges!='' && $scope.case_closing.transportation_charges!='undefined'){
            ...
            for(var i=0;i<$scope.case_closing.transportation_charges.length;i++){
                if($scope.case_closing.transportation_charges[i].tax_id!=null && $scope.case_closing.transportation_charges[i].tax_id!='' && $scope.case_closing.transportation_charges[i].tax_id!='undefined'){
                    // $scope.tax_data[i]=JSON.parse($scope.case_closing.transportation_charges[i].tax_id);
                    $scope.tax_data[i]=$scope.case_closing.transportation_charges[i].tax_id;
    ...
    $scope.calculate_total = function(tax){
        ...
        if(tax != undefined){
            ...
            $scope.case_closing.tax_id = $scope.tax_data.id;
            console.log($scope.case_closing.tax_id);
            $scope.case_closing.tax_rate = $scope.tax_data.tax_rate;
            console.log($scope.case_closing.tax_rate);
            $scope.case_closing.tax_amount = parseFloat($scope.case_closing.close_towing_charges) * parseFloat($scope.case_closing.tax_rate) / 100;
            console.log($scope.case_closing.close_towing_charges);
        }else{
            $scope.case_closing.tax_amount = 0;
        }

        $scope.case_closing.subtotal = parseFloat($scope.case_closing.close_towing_charges) + parseFloat($scope.case_closing.tax_amount);
        $scope.case_closing.total = $scope.case_closing.subtotal;
    }
}]);