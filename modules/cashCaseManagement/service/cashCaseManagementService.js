caseManagementModule.factory('getCashCaseDetailsList',function($http){
    return function (params) {
        return $http.get(appUrl+'get_cash_case_listing?token='+localStorage.getItem('token')+'&company_id='+localStorage.getItem('warehouse_id'),{params:params,header:{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8'}
        })
    }
...
}).factory('getServiceList1',function($http){
    return function (params) {
        return $http.get(appUrl+'get_service_or_rule_listing?token='+localStorage.getItem('token'),{params:params,header:{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8'}
        })
    }
