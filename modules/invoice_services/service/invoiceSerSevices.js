...
}).factory('getServicesAutocomplete',function($http){
    return function (params) {
        return $http.get(appUrl+'services_autocomplete?token='+localStorage.getItem('token'),{params:params,header:{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8'}
        })
    }
...
}).factory('InvoiceServiceListingService',function($http){
    return function (params) {
        return $http.get(appUrl+'get_invoice_service_listing?token='+localStorage.getItem('token'),{params:params,header:{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8'}
        })
    }
