...
userManagementModule.controller('userManagementController',['$rootScope','$scope','$location','getUserManagementService','getUsersListingService','saveUserPermissionService','userPermissionMenuService','updateUserPermissionService',function($rootScope,$scope,$location,getUserManagementService,getUsersListingService,saveUserPermissionService,userPermissionMenuService,updateUserPermissionService){
        ...
        for(var p=0;p<$scope.software_all_rights.length;p++){
            if($scope.software_all_rights[p].modules_name=="Reports"){
                ...
                for(var g=0;g<$scope.software_all_rights[p].sub_modules.length;g++){
                    ...
                    if ($scope.software_all_rights[p].sub_modules[g].sub_sub_modules.length==0){

                        $scope.add_sub_module = {};

                        $scope.add_sub_module.id = $scope.software_all_rights[p].sub_modules[g].id;
                        $scope.add_sub_module.module_id = $scope.software_all_rights[p].sub_modules[g].module_id;
                        $scope.add_sub_module.sub_module_name = $scope.software_all_rights[p].sub_modules[g].modules_name;

                        $scope.software_all_rights[p].sub_modules[g].sub_sub_modules.push($scope.add_sub_module);
                    }
        ...
        if(ssm_id == undefined) {
                                ...
                                for (var f = 0; f < $scope.add_user_management.module_permissions[g].sub_modules.length; f++) {
                                    if ($scope.add_user_management.module_permissions[g].sub_modules[f].sub_module_id == sub_mod_id) {
                                        ...
                                        if (f == $scope.add_user_management.module_permissions[g].sub_modules.length - 1 && $scope.add_user_management.module_permissions[g].sub_modules[f].sub_module_id != sub_mod_id) {
                                            ...
                                            $scope.sub_modules.sub_sub_module = [];
            ...
            if (document.getElementById(type + '_' + sub_mod_id+ssm_id).checked) {
                if ($scope.add_user_management.module_permissions.length > 0) {
                            ...
                            if ($scope.add_user_management.module_permissions[g].module_id == mod_id) {
                                ...
                                for (var f = 0; f < $scope.add_user_management.module_permissions[g].sub_modules.length; f++) {
                                    if ($scope.add_user_management.module_permissions[g].sub_modules[f].sub_module_id == sub_mod_id) {
                                        ...
                                        //$scope.add_user_management.module_permissions[g].sub_modules[f].sub_sub_module = [];
                    
                                        if($scope.add_user_management.module_permissions[g].sub_modules[f].hasOwnProperty('sub_sub_module')==false){
                                            $scope.add_user_management.module_permissions[g].sub_modules[f].sub_sub_module = [];
                                        } 

                                        $scope.ssm_data_obj = {};
                    
                                        $scope.ssm_data_obj.sub_sub_module_id = ssm_id;
                                        $scope.ssm_data_obj.sub_module_id = sub_mod_id;
                                        $scope.ssm_data_obj.add_option = 'no';
                                        $scope.ssm_data_obj.edit_option = 'no';
                                        $scope.ssm_data_obj.delete_option = 'no';
                    
                                        $scope.add_user_management.module_permissions[g].sub_modules[f].sub_sub_module.push($scope.ssm_data_obj);
                                        ...
                                        if (f == $scope.add_user_management.module_permissions[g].sub_modules.length - 1) {
                                            ...
                                            $scope.sub_modules.sub_sub_module.push($scope.smm_data_obj);

                                            if($scope.sub_modules.sub_sub_module[0]==undefined){
                                                $scope.sub_modules.sub_sub_module.splice(0,1);
                                            }  
                                ...
                                if (g == $scope.add_user_management.module_permissions.length - 1) {
                                    ...
                                    $scope.modules.sub_modules.sub_sub_module = [];
                                    ...
                                    $scope.modules.sub_modules.sub_sub_module.push($scope.smm_data_obj);
                    ...
                    $scope.sub_modules.sub_sub_module.push($scope.ssm_data_obj);                   
        ...
        if(ssm_id==undefined) {
            ...
            if (document.getElementById('submodule_option_' + sub_mod_id).checked) {
                ...
                if (type != 'Debit Note' && type != 'Stock Entry' && type != 'Stock List' && type != 'Stock Exit' && type != 'Credit Note' && type != 'Debit Note' && type != 'Balance Sheet' && type != 'Profit & Loss' && type != 'Bank Account' && type != 'Payroll' && type != 'Attendance' && type != 'Otp List' && type != 'Log File' && type != 'Purchase' && type != 'Inventory' && type != 'Sale' && type != 'Daily Report' && type != 'Bank Accounts' && type != 'OTP List' && type != 'Log Files' && type != 'Printable Format' && type != 'Backup Database' && type != 'Daily Report Reports') {
                    ...
                    document.getElementById('view_option' + '_' + sub_mod_id).checked = true;
                    ...
                    for (var e = 0; e < 4; e++) {
                        ...
                        if (e == 3) {
                            $scope.add_permission_function(mod_id, sub_mod_id, 'view_option');
                        }
                    }
                ...
                if (type != 'Debit Note' && type != 'Stock Entry' && type != 'Stock List' && type != 'Stock Exit' && type != 'Credit Note' && type  != 'Debit Note' && type != 'Balance Sheet' && type != 'Profit & Loss' && type != 'Bank Accounts' && type != 'Payroll' && type != 'Attendance' && type != 'OTP List' && type != 'Log Files' && type != 'Backup Database' && type!='Printable Format' && type != 'Purchase' && type != 'Inventory' && type != 'Sale' && type != 'Daily Report' && type != 'Daily Report Reports') {
                    ...
                    document.getElementById('view_option' + '_' + sub_mod_id).checked = false;
    ...
    $scope.wholeModule = function (mod_id) {
        ...
        console.log(mod_id);
        console.log(document.getElementById('wholemodule_option_'+mod_id).checked);
    ...
    $scope.reportWholeModule = function (sub_id,mod_id) {

        if(document.getElementById('wholemodule_option_'+sub_id+mod_id).checked){

            for(var n=0;n<all_modules.length;n++) {

                if (all_modules[n].id == sub_id && all_modules[n].module_id == mod_id) {

                    if (all_modules[n].sub_modules) {

                        for (var m = 0; m < all_modules[n].sub_modules.length; m++) {

                            if(all_modules[n].sub_modules[m].id + all_modules[n].sub_modules[m].sub_sub_module_id){

                                document.getElementById('submodule_option_' + all_modules[n].sub_modules[m].id + all_modules[n].sub_modules[m].sub_sub_module_id).checked = true;

                                $scope.submodule_permission_function(mod_id,all_modules[n].sub_modules[m].id,all_modules[n].sub_modules[m].sub_module_name,all_modules[n].sub_modules[m].sub_sub_module_id);
                            }else{
                                $scope.add_permission_function(mod_id, sub_id, 'submodule_option');
                            }
                        }
                    }
                }
                    ...
                    if (all_modules[n].sub_modules) {
                        ...
                        for (var m = 0; m < all_modules[n].sub_modules.length; m++) {
                            ...
                            if(all_modules[n].sub_modules[m].id + all_modules[n].sub_modules[m].sub_sub_module_id){
                                ...
                                document.getElementById('submodule_option_' + all_modules[n].sub_modules[m].id + all_modules[n].sub_modules[m].sub_sub_module_id).checked = false;

                                for(var k=0;k<$scope.add_user_management.module_permissions.length;k++){

                                    for(var z=0;z<$scope.add_user_management.module_permissions[k].sub_modules.length;z++){

                                        if($scope.add_user_management.module_permissions[k].sub_modules[z].module_id==mod_id && $scope.add_user_management.module_permissions[k].sub_modules[z].sub_module_id==sub_id){

                                            $scope.add_user_management.module_permissions[k].sub_modules.splice(z,1);
                                        }
                                    }
                                }
                            }else{
                                document.getElementById('submodule_option_' + all_modules[n].sub_modules[m].id).checked = false;
                            }
                        }
                    }
                }
    ...
    $scope.get_user_permission_all = function () {

        userPermissionMenuService({user_id:$scope.add_user_management.user_id,type:'user'}).then(function (res) {
           ...
           if(res.data.length>0){
                ...
                for(var n=0;n<res.data.length;n++){
                    ...
                    for(var l=0;l<res.data[n].user_modules[0].user_role_sub_modules.length;l++){

                        for(var key in res.data[n].user_modules[0].user_role_sub_modules[l]) {

                            var value = res.data[n].user_modules[0].user_role_sub_modules[l][key];

                            if(value=='yes'){

                         console.log(res.data[n].user_modules[0].user_role_sub_modules[l]);

                                if(res.data[n].user_modules[0].user_role_sub_modules[l].sub_sub_module_id==0){

                                    if(res.data[n].user_modules[0].user_role_sub_modules[l].hasOwnProperty('user_role_sub_sub_modules') && res.data[n].user_modules[0].user_role_sub_modules[l].user_role_sub_sub_modules.length>0){

                                        for(c=0;c<res.data[n].user_modules[0].user_role_sub_modules[l].user_role_sub_sub_modules.length;c++){

                                            document.getElementById(key + '_' + res.data[n].user_modules[0].user_role_sub_modules[l].sub_module_id + res.data[n].user_modules[0].user_role_sub_modules[l].user_role_sub_sub_modules[c].sub_sub_module_id).checked = true;        
                                            
                                            $scope.add_permission_function(res.data[n].user_modules[0].user_role_sub_modules[l].modules_id,res.data[n].user_modules[0].user_role_sub_modules[l].sub_module_id,key,res.data[n].user_modules[0].user_role_sub_modules[l].user_role_sub_sub_modules[c].sub_sub_module_id);                                    
                                        }
                                    }else{
                                        
                                    document.getElementById(key + '_' + res.data[n].user_modules[0].user_role_sub_modules[l].sub_module_id).checked = true;
                                    
                                    $scope.add_permission_function(res.data[n].user_modules[0].user_role_sub_modules[l].modules_id,res.data[n].user_modules[0].user_role_sub_modules[l].sub_module_id,key);
                                    }
                                }/*else{

                                    console.log(key);

                                    document.getElementById(key + '_' + res.data[n].user_modules[0].user_role_sub_modules[l].sub_module_id + res.data[n].user_modules[0].user_role_sub_modules[l].sub_sub_module_id).checked = true;

                                    $scope.add_permission_function(res.data[n].user_modules[0].user_role_sub_modules[l].modules_id,res.data[n].user_modules[0].user_role_sub_modules[l].sub_module_id,key,res.data[n].user_modules[0].user_role_sub_modules[l].sub_sub_module_id);
                ...
                $scope.show_update_btn = true;
            }
        });
    };

    $scope.update_user_management = function () {

        updateUserPermissionService({permissions:$scope.add_user_management}).then(function (res) {
            $('#permissionSuccess').fadeIn().delay(5000).fadeOut();
            $scope.permission_success_msg = JSON.parse(res.data);
            $scope.add_user_management = {module_permissions:[]};
            $('input:checkbox').removeAttr('checked');
            $scope.show_save_btn = false;
            $scope.show_update_btn = false;
        },function (res) {
            console.log(res);
        })
    };
